...
import { ZabbixAPI, AsyncZabbixAPI, Sender, AsyncSender, Getter, AsyncGetter, APIVersion, ItemValue } from '../../dist';
...
class CompatibilityGetTest {
    private host: string;
    private port: number;
    private agent: Getter | null = null;
    ...
    setUp(): void {
        this.agent = new Getter({
            host: this.host,
            port: this.port
        });
    }

    async testGetValues(): Promise<void> {
        console.log('Testing Zabbix 5.0 getter...');
        if (!this.agent) throw new Error('Getter not initialized');
...
async function runAllZabbix50CompatibilityTests(): Promise<void> {
    try {
